  ...
  return blocks.map((block, i) => {
    ...
    if (code) {
      return (
        <pre
          key={i}
          className="my-1 overflow-x-auto rounded bg-[#1e1f22] p-2 font-mono text-sm text-discord-text"
        >
          <code>{code[1].replace(/\n$/, "")}</code>
        </pre>
      );
...
function renderInline(text: string): React.ReactNode[] {
  ...
  while ((m = pattern.exec(text))) {
    ...
    const tok = m[0];
    if (tok.startsWith("`")) out.push(<code key={key} className="rounded bg-[#1e1f22] px-1 py-0.5 font-mono text-sm">{tok.slice(1, -1)}</code>);
    ...
    else if (tok.startsWith("http")) out.push(<a key={key} href={tok} target="_blank" rel="noreferrer" className="text-[#00a8fc] hover:underline">{tok}</a>);
    key++;
    ...
    <span
      ...
      className={
        revealed
          ...
          : "cursor-pointer rounded bg-[#1e1f22] px-0.5 text-transparent"
      }
    >
      {children}
    </span>
